...
void Problem084::chance(std::vector<int> &cards, int &location) {
    ...
    else if (cards[0] == 7 || cards[0] == 8) location = (location < 5 ? 5 : location < 15 ? 15 : location < 25 ? 25 : 35);
    else if (cards[0] == 9) location = location < 12 ? 12 : 28;
...
std::string Problem084::getAnswer() {
    ...
    for (int turns = 0; turns < 10000000; ++turns) {
        ...
        else if (location == 30) location = 10; //Go to jail
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        std::cout << "Card: " << (pr->first < 10 ? "0" : "") + std::to_string(pr->first) << std::endl;
        counts.erase(pr);
    }
